  ...
  <div>
    ...
    <el-card style="padding: 10px">
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column property="policeId" label="关联民警">
          <template #default="scope">
            {{ getName(scope.row.policeId) }}
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 获取列表
    getList() {
      baseService.post("/school/pageList", this.form).then((res) => {
    ...
    // 删除
    confirm(val) {
      baseService.delete(`/school/delete/${val.id}`).then((res) => {
        if (res.code === 0) {
