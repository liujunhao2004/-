<template>
  <el-dialog title="更新" width="70%" v-model="visible" :before-close="handleClose">
    ...
    submitForm() {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          try {
            const response = await baseService.post("exam/api/answering/task/update", queryForm);
            this.$notify({ title: "成功", message: "更新成功", type: "success" });
            this.close("form");
            this.$emit("refresh");
          } catch (err) {
            this.$notify.error({ title: "失败", message: err.message || "更新失败" });
          } finally {
          }
        } else {
          return false;
