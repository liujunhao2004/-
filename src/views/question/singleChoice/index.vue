        ...
        <el-row :gutter="20" type="flex" justify="space-between">
          ...
          <el-col :span="6">
            <el-form-item label="题目内容">
              <el-input v-model="requestForm.params.content"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="题目类型">
              <el-select v-model="requestForm.params.questionType" placeholder="请选择" style="width: 100%">
    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column prop="questionType" label="题目类型" width="100">
          <template v-slot="scope">
            {{ getDictionary(questionType, scope.row.questionType, "code", "name") }}
          </template>
        </el-table-column>

        <el-table-column label="题目内容">
          <template v-slot="scope">
            <span class="texts">{{ scope.row.content }}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    // isParamsAllEmpty() {
    //   const { content, questionType, tagId } = this.requestForm.params;
    //   return (content === "" || content === undefined || content === null) && (questionType === "" || questionType === undefined || questionType === null) && (tagId === "" || tagId === undefined || tagId === null);
    // },
    // 查询
    query() {
    ...
    // 获取列表
    async getList() {
      const res = await baseService.post("exam/api/question/listPage", this.requestForm);
    ...
    async deleteFn(val) {
      try {
        await baseService.post("exam/api/question/delete", { id: val.id });
        this.$notify({
          title: "删除成功",
          type: "success",
          duration: 2000
