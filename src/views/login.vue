<template>
  <div class="rr-login">
    ...
    <div class="rr-login-wrap">
      <div class="rr-login-left hidden-sm-and-down" style="display: none">
        <p class="rr-login-left-title">高校平安宣传码</p>
      </div>

      <div class="rr-login-right">
        <div class="rr-login-right-main">
          <h4 class="rr-login-right-main-title" style="margin-bottom: 4px">登录</h4>
          <p class="rr-login-right-main-title" style="margin-bottom: 27px; font-size: 11px">欢迎使用高校平安宣传码</p>
          <el-form ref="formRef" label-width="80px" :status-icon="true" :model="login" :rules="rules" @keyup.enter="onLogin">
            ...
            <el-form-item label-width="0" prop="captcha">
              <el-space class="rr-login-right-main-code">
                ...
                <img style="vertical-align: middle; height: 32px; cursor: pointer" :src="state.captchaUrl" @click="onRefreshCode" alt="" />
              </el-space>
            </el-form-item>
            <el-form-item label-width="0">
              <el-button type="primary" loading-icon="Lollipop" :loading="state.loading" @click="onLogin" class="gradient-btn primary"> 登录 </el-button>
            </el-form-item>
          </el-form>
          <p class="textali">该系统由：京兆三维码提供技术支持</p>
        </div>
      </div>
    </div>
  </div>
...
::v-deep .gradient-btn.primary {
  width: 100%;
  background: linear-gradient(135deg, #64fffd, #3968ff);
  border: 0px;
}
