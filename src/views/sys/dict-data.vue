<template>
  <div class="mod-sys__dict">
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column label="icon" header-align="center" align="center">
        <template v-slot="scope">
          <img v-if="scope.row.icon" :src="scope.row.icon" alt="" style="width: 75px; height: 75px" />
          <span v-else>/</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="备注" header-align="center" align="center">
        <template v-slot="scope">
          <span v-if="scope.row.remark" class="texta">{{ scope.row.remark }}</span>
          <span v-else>/</span>
        </template>
      </el-table-column>
