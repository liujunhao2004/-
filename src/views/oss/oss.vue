<template>
  <div class="mod-oss__oss">
    <el-form :inline="true" :model="state.dataForm">
      <el-form-item> <el-input v-model="input" style="width: 240px" placeholder="请输入文件名：" @input="searchFor" /> </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column label="文件名称" header-align="center" align="center">
        <template v-slot="scope">
          <span class="texta">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
...
const debouncedSearch = debounce(() => {
  state.pageSizeChangeHandle(state.limit, state.name);
}, 300);
onMounted(() => {
  return () => {
